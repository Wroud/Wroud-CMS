...
function POST($patch, $POST) {
    switch ($POST['send']) {
        case'game':
            ...
            if (strlen($POST['gamepassword']) < 3 || strlen($POST['gamepassword']) > 16)
                return json_encode(array(false, TPL_LANGUAGE::$messages[2]));
            $realm = 0;
            foreach (TPL_INDEX::$realms as $rea)
                if (isset($POST['realm']) && $rea['title'] == $POST['realm'])
                    $realm = $rea['id'];
            if (!$realm)
                return json_encode(array(false, TPL_LANGUAGE::$messages[5]));
            if (!SCL_DATABASE::query(SQL_SET_ACCOUNT, TPL_INDEX::$realms[$realm]['auth'], TPL_INDEX::$USER['username'], sha1($POST['gamepassword']), TPL_INDEX::$USER['email']))
        ...
        case'off':
            ...
            break;
        case'login':
            $user = SCL_DATABASE::selectRow(SQL_GET_USER, $POST['username']);
