...
var Slideshow = {
    ...
    initialize: function(object, data) {
        ...
        var slideshow = Slideshow.object.find('.slideshow');
        var padding = Slideshow.object.find('.paging');
        var caption = Slideshow.object.find('.caption');
        for(var i=0;i<data.length;i++){
            slideshow.append('<div class="slide" id="slide-'
                +i
                ...
                +';"></div>')
            .fadeIn(1500);
            padding.append('<a href="javascript:;" id="paging-'
                +i
                +'" onclick="Slideshow.jump('
                +i
                +', this);" onmouseover="Slideshow.preview('
                +i
                +');" class="'
                +((i==0)?'current':((i==(data.length-1))?'last-slide':''))
                +'"></a>')
            if(i==0)
                caption.append('<h3><a href="'
                    +data[i]['url']
                    +'" class="link">'
                    +data[i]['title']
                    +'</a></h3>'
